...
╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
 ...
 Bartek ma zdefiniowany problem M2 (docs/Plans/EVSO_Milestone2_Problem_Definition.md) — ciągłość konwersacji email ↔
 ClickUp, problemy A–F, 7 kryteriów sukcesu, świadome uproszczenia (np. email = tożsamość klienta). Wcześniejsze próby
 rozwiązania (specyfikacja, meeting notes, ocena agentów ClickUp, "jak rozwiązujemy") zostały przeniesione do archive/ —
 Bartek świadomie odcina się od tych iteracji i chce zaprojektować rozwiązanie od zera, żeby uniknąć biasu i lock-in'u na
 kierunku, który wcześniej wyszedł.

 Bartek nie prosi o rozwiązanie. Prosi o workflow — strukturalny proces myślowy, który gwarantuje dotarcie do dobrego,
 spójnego, konkretnego rozwiązania. Wynik workflow ma być Implementation Guide (gotowy do ręcznego wdrożenia w Make.com +
 ...
 2. Tryb wykonania: Faza-po-fazie, osobne sesje. Każda faza = jeden prompt do Claude → jeden plik markdown → review przez
 Bartka → korekta → przejście do następnej.
 ...
 Każda faza ma być zaprojektowana tak, żeby wymuszała konkretny proces myślowy, którego nie da się skrócić. Bez tych
 wymuszeń pierwsza intuicja wygrywa i tracimy szansę na lepsze rozwiązanie.

 1. Anti-bias guard na starcie każdej sesji. Pierwsze zdanie outputu Claude w każdej fazie: explicit ack listy plików, które
  wczytał, i potwierdzenie "nie czytałem archive/". Brak tego = faza odrzucona.
 2. Divergencja przed konwergencją. Faza 3 wymusza 3 zasadniczo różne kierunki architektoniczne — nie warianty tego samego
 pomysłu. Pierwszy pomysł nie wygrywa automatycznie.
 ...
 nie-konsensus.
 4. Adversarial gate przed lock-in. Faza 6 świadomie próbuje zburzyć projekt z Fazy 5. Lepiej znaleźć dziurę teraz niż w
 produkcji.
 5. Każda faza ma kryterium gotowości. Bez "wydaje mi się że wystarczy" — jest konkretny test, który mówi czy faza
 skończona.
 6. Outputy są kumulatywne. Każda kolejna faza wczytuje outputy poprzednich. Nie wracamy do problem definition w każdej
 fazie — Faza 0 robi z niej trwałą "mapę pracy".
 7. Mapping back na końcu. Faza 7 explicit mapuje rozwiązanie na 7 kryteriów sukcesu z Problem Definition. Loop closure
 obowiązkowy.
 ...
 Język outputów: PL (zgodny z Problem Definition i kontekstem zespołu). Nazwy techniczne (Custom Fields, scenariusze Make,
 statusy) verbatim zgodnie z CLAUDE.md.
 ...
 Cel poznawczy: Zinternalizować problem własnymi słowami, bez biasów z poprzednich iteracji. Stworzyć trwałą mapę problemu,
 która będzie wczytywana przez wszystkie kolejne fazy.
 ...
 Zadanie:
 1. Przepisać problemy A–F własnymi słowami — każdy w 3 sekcjach: istota, przykład z życia handlowca EVSO, konsekwencja
 jeśli zostanie nierozwiązany.
 ...
 3. Wylistować świadome kompromisy z Problem Definition (np. email = tożsamość, telefon = manual) i dla każdego — co
 dokładnie tracimy decyzją upraszczającą.
 4. Wylistować anti-assumptions — czego rozwiązanie NIE może zakładać (klient nie zachowuje się przewidywalnie; nie każdy
 email da się jednoznacznie powiązać; itp.).

 Output: 01_Phase0_Problem_Map.md

 Kryterium gotowości: Każdy z 6 problemów (A–F) ma swoją sekcję; każdy świadomy kompromis ma jasny "koszt"; lista
 anti-assumptions zawiera ≥4 punkty.
 ...
 1. Inwentaryzacja zdolności tooling'u w 4 obszarach:
   - ClickUp Business: Email-to-Task (per-task @mg.clickup.com), AI Agents, AI Fields, Custom Fields, Automations, Brain,
 Inbox, native email composer, ClickUp Forms.
   - Make.com: data stores, scenariusze, watchery (Email/Gmail), routery, error handlers, Iterators/Aggregators, scheduled
 vs instant triggers.
   - Anthropic API (Claude Haiku 4.5): klasyfikacja, ekstrakcja, semantyczne dopasowanie, agentyczne loopy (multi-turn),
 tool use, prompt caching.
   - Vibe coding (microservice): kiedy ma sens (niestandardowa logika fuzzy matchingu, custom UI, persistent state) i kiedy
 NIE (gdy ClickUp/Make to robi natywnie).
 2. Dla każdej zdolności podać: co potrafi, gdzie ma limity / pułapki, koszt operacyjny (rate limits, ceny).
 3. Zbudować Capability × Problem matrix — tabela 6 problemów × 4 obszary tooling. Każda komórka: "tak, adresuje, jak…" /
 "częściowo, brakuje…" / "nie, wymaga workaround".

 Output: 02_Phase1_Capability_Map.md

 Kryterium gotowości: Każdy z 6 problemów ma ≥1 mapping na konkretną zdolność. Capability matrix wypełniona (24 komórki).
 Sekcja "limity i pułapki" jest niefantazjowana — oparta na rzeczywistych dokumentacjach lub własnym doświadczeniu z Phase
 1.
 ...
 - Polityka powracających klientów (problem z Problem Definition: lead z zeszłego roku ≠ nowy wyjazd) — okno czasu, po
 którym nowy email = nowy task.
 ...
 Kryterium gotowości: 6 wierszy (A–F) × 1 decyzja każdy + 4 polityki rozstrzygnięte. Każda decyzja ma uzasadnienie ≥2
 zdania.

 ---
 Faza 3 — Divergent Solution Sketches (3 niezależne kierunki)

 Cel poznawczy: Nie pozwolić pierwszemu pomysłowi wygrać. Wymuszone wygenerowanie 3 zasadniczo różnych kierunków daje widok
 na space rozwiązań i ujawnia ukryte trade-offy.

 Input: Fazy 0, 1, 2.

 Zadanie: Naszkicować 3 kierunki, zasadniczo różne (nie warianty siebie):

 - Kierunek A — "ClickUp-native heavy": ClickUp Agents + Email-to-Task + Automations robią większość pracy. Make jest cienką
  warstwą I/O. AI Fields klasyfikują, Agents matchują.
 - Kierunek B — "Make-watcher heavy": Make jest centralnym mózgiem. Watcher Gmaila → klasyfikator AI w Make → router →
 ClickUp dostaje gotowe instrukcje (utwórz/przypisz/notyfikuj). ClickUp jest prawie pasywnym storage + UI.
 ...
 Kryterium gotowości: 3 kierunki są ZASADNICZO różne (jeśli dwa zaczynają wyglądać podobnie — przeprojektować jeden). Każdy
 ma 6 sekcji wyżej. Żaden nie jest "słomianym ludkiem" (każdy musi być realistycznie obroniony).

 ---
 Faza 4 — Expert Panel Stress Test

 Cel poznawczy: Nie dopuścić, żeby kierunek wygrał na bazie estetyki. Wymusić cross-examination z 4 specjalistycznych
 perspektyw.
 ...
 - Marta Kurek (ClickUp Systems Architect) — czy kierunek pasuje do ziarna ClickUp? gdzie automacje się zaczną łamać przy
 skali? jaka jest "ergonomia handlowca" w tym kierunku?
 - Tomasz Sowa (Make Automation Architect) — co jest stabilne/kruche w Make w tym kierunku? jakie są scenariusze błędów?
 error handling?
 - Aleksander Bryła (AI Systems & Reliability) — gdzie AI w tym kierunku można ufać? gdzie zawodzi po cichu? jak wygląda
 confidence gating?
 - Karolina Wysocka (Sales & Service Process Designer) — który kierunek faktycznie ułatwia handlowcowi dzień? gdzie są nowe
 friction points? jak ten kierunek interaguje z istniejącym sales process?

 Zasada zespołu (z EVSO_DevTeam.md): "tylko propozycja, która jest prostsza, stabilniejsza lub faktycznie bardziej użyteczna
  niż alternatywa".
 ...
 Kryterium gotowości: 4 rankingi × 3 pozycje. Decision Record podpisany "konsensus" lub "rozstrzygnięty arbitraż" (jasne kto
  i czemu wygrał spór). Lista nie-konsensusu nie jest pusta (jeśli jest — panel pracował zbyt grzecznie).

 ---
 Faza 5 — Architecture Lock-In + Edge Cases

 Cel poznawczy: Wziąć zwycięski kierunek i go skonkretyzować do poziomu, na którym każdy scenariusz brzegowy ma odpowiedź.
 Nie wolno zostawić "to później rozkminimy".
 ...
 Definition i pytamy: "jakie sytuacje muszą być pokryte żeby spełnić ten warunek". Spodziewane 6–10 scenariuszy. Dla
 każdego: wejście, trigger, kroki systemu, output, fallback przy błędzie.
 3. Data model changes — co dodajemy do ClickUp (nowe Custom Fields, Automations); co dodajemy do Make (data stores,
 scenariusze); ewentualne schemata mikroservisu (jeśli Kierunek C wygrał).
 4. Failure modes — co system robi gdy: AI klasyfikuje błędnie / niżej niż próg, email nie pasuje do żadnego taska, są
 duplicates, klient zmienił adres, klient wraca po roku z nowym wyjazdem, ClickUp API jest down, Gmail watcher gubi
 wiadomość.
 5. Migration plan — jak włączyć paralelnie z Phase 1 (nie burząc istniejącego form intake) w testowym środowisku
 (Forminator klon + automatyzacjaevso@gmail.com + testowy ClickUp Workspace). Kolejność włączania.

 Output: 06_Phase5_Architecture.md

 Kryterium gotowości: Każdy z 7 kryteriów sukcesu ma ≥1 mapowany scenariusz Sx. Każdy scenariusz Sx ma fallback. Migration
 plan jest etap-po-etapie (nie "włączamy wszystko naraz").
 ...
 - Zmiana adresu klienta: klient pisze najpierw z jan@firma.pl, potem z jan@gmail.com. Wiemy, że nie obsługujemy tego
 (świadomy kompromis) — ale jaki dokładnie efekt? Duplikat? Nowy task? Manual cleanup?
 - Powrót po roku: klient pisze rok później na nowy wyjazd. System rozpoznaje że to "ten sam email" — czy poprawnie tworzy
 NOWY task, nie wrzuca do archiwalnego?
 - Gmail outage: watcher Make nie odbiera wiadomości przez 6h. Czy system się leczy po przywróceniu? Czy gubimy wiadomości?
 - Operability: Bartek wyjeżdża na 2 tygodnie. Czy zespół (handlowcy bez technicznej wiedzy) może utrzymać system? Co jeśli
 Make scenariusz się zatrzyma — kto i jak diagnozuje?

 Output: 07_Phase6_Adversarial_Review.md

 Kryterium gotowości: ≥8 ataków. Każdy ma decyzję (nie wolno "rozważymy"). Lista fix-now ma być wbudowana z powrotem do Fazy
  5 (update 06_Phase5_Architecture.md) przed przejściem do Fazy 7.

 ---
 Faza 7 — Implementation Guide (Final Deliverable)

 Cel poznawczy: Złożyć wszystko w dokument, który Bartek może wziąć i wdrożyć bez wracania do wcześniejszych dokumentów
 workflow.
 ...
 1. Cele — mapping na 7 kryteriów sukcesu z EVSO_Milestone2_Problem_Definition.md. Dla każdego kryterium: jak guide je
 realizuje + jak to przetestować.
 ...
 4. Scenariusze Make — module-by-module, z konkretnymi modułami (Webhook / Gmail Watcher / Anthropic / ClickUp / Iterator / Router / Error Handler). Per-scenariusz: trigger, moduły w
 Cel poznawczy: Świadomie zburzyć projekt z Fazy 5. Lepiej znaleźć dziurę teraz niż na żywym kliencie.

 Input: Faza 5.

 Zadanie: Lista ataków na projekt. Dla każdego: opis sytuacji, co się stanie wg projektu, czy to jest OK, decyzja (fix-now / accept-risk/ fix-later
  z tagiem). Lista minimalna (każdy musi być rozważony):
 - Skala 10x: 1000 leadów/tydzień zamiast 100. Co się łamie? Rate limits? Koszt API? Manual queue rośnie zbyt szybko?
 - Cicha porażka AI: klasyfikator zwraca wysokie confidence ale błędną decyzję. Jak to wykrywamy? Po jakim czasie?
 - Pracownik ignoruje: notyfikacje są ignorowane przez tydzień. Co system robi? Eskaluje?
 - Zmiana ClickUp API: @mg.clickup.com przestaje działać / format się zmienia. Jak system się degraduje?
 - Zmiana adresu klienta: klient pisze najpierw z jan@firma.pl, potem z jan@gmail.com. Wiemy, że nie obsługujemy tego (świadomy kompromis) — ale
 jaki dokładnie efekt? Duplikat? Nowy task? Manual cleanup?
 - Powrót po roku: klient pisze rok później na nowy wyjazd. System rozpoznaje że to "ten sam email" — czy poprawnie tworzy NOWY task, nie wrzuca do
 archiwalnego?
 - Gmail outage: watcher Make nie odbiera wiadomości przez 6h. Czy system się leczy po przywróceniu? Czy gubimy wiadomości?
 - Operability: Bartek wyjeżdża na 2 tygodnie. Czy zespół (handlowcy bez technicznej wiedzy) może utrzymać system? Co jeśli Make scenariusz się
 zatrzyma — kto i jak diagnozuje?

 Output: 07_Phase6_Adversarial_Review.md

 Kryterium gotowości: ≥8 ataków. Każdy ma decyzję (nie wolno "rozważymy"). Lista fix-now ma być wbudowana z powrotem do Fazy 5 (update
 06_Phase5_Architecture.md) przed przejściem do Fazy 7.
 ...
 1. Cele — mapping na 7 kryteriów sukcesu z EVSO_Milestone2_Problem_Definition.md. Dla każdego kryterium: jak guide je realizuje + jak to
 przetestować.
 2. Architektura wysokopoziomowa — diagram komponentów + dataflow + integration points (z Fazy 5).
 3. Zmiany w ClickUp — additive, nie destruktywne (zasada z CLAUDE.md):
   - Nowe Custom Fields (z polskimi nazwami).
   ...
   - Email-to-Task config / @mg.clickup.com setup.
   - AI Agents config (jeśli używamy).
 4. Scenariusze Make — module-by-module, z konkretnymi modułami (Webhook / Gmail Watcher / Anthropic / ClickUp / Iterator / Router / Error Handler).
  Per-scenariusz: trigger, moduły w kolejności, mapping zmiennych, error handling.
 5. Prompty AI (versioned) — pełne prompty (system + user + przykłady), z numerem wersji v1.0. Zachować strukturę KLASYFIKACJA_AI / AI_CONFIDENCE
   - Email-to-Task config / @mg.clickup.com setup.
   - AI Agents config (jeśli używamy).
 ...
 5. Anti-bias verification: Żaden plik wynikowy w docs/Plans/M2_Workflow/ nie cytuje, nie referuje ani nie kopiuje treści z archive/*.
 Można sprawdzić Grep'em.
